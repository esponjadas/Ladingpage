        ...
        <Reveal>
          <div className="max-w-[28rem]">
            ...
            <p className="mt-6 text-[1.02rem] leading-[1.84] text-white/62 md:text-lg">
              Estamos construindo o Kerso com calma, precisão e foco em quem quer uma relação
              melhor com o próprio dinheiro.
            </p>
            ...
            return (
              <Reveal key={item.question} delay={index * 0.06}>
                ...
                >
                  ...
                  <button
                    className="relative flex min-h-14 w-full items-center justify-between gap-4 px-6 py-5 text-left md:px-7"
                    onClick={() => setOpenIndex(isOpen ? -1 : index)}
                    aria-expanded={isOpen}
                  >
                    <span className="font-display text-[1.12rem] font-[740] tracking-[-0.03em] text-white md:text-[1.35rem]">
                      {item.question}
                    </span>
                    <motion.span
                    ...
                    >
                      <ChevronDown
                        className="text-white/58 transition group-hover:text-signal"
                        size={20}
                      />
                    </motion.span>
                  </button>
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen ? (
                      ...
                      >
                        <div className="relative px-6 pb-6 text-[0.98rem] leading-[1.82] text-white/62 md:px-7">
                          {item.answer}
                        </div>
                      </motion.div>
                    ) : null}
